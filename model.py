...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, optm, lr, lr_muon, device_type, master_process):
        """
        ...
        if optm == "AdamW":
            ...
            from optimizers.nomalized_opt import NormalizedSGD
            optimizer = NormalizedSGD(self.parameters(), lr=lr, momentum=0.9, weight_decay=weight_decay)
        elif optm == "BNSGD":
            from optimizers.nomalized_opt import BlockNormalizedSGD
            optimizer = BlockNormalizedSGD(self.parameters(), lr=lr, momentum=0.9, weight_decay=weight_decay)
        elif optm == "muon":
            ...
            def _is_embedding(name: str):
                return name.startswith('transformer.wte') or name.startswith('transformer.wpe')

            def _is_lm_head(name: str):
                return name.startswith('lm_head')

            param_dict = {pn: p for pn, p in self.named_parameters() if p.requires_grad}
            hidden_weights = [
                p for n, p in param_dict.items()
                if p.ndim in (2, 3, 4) and not _is_embedding(n) and not _is_lm_head(n)
            ]
            hidden_gains_biases = [p for n, p in param_dict.items() if p.ndim < 2]
            nonhidden_params = _unique([
                p for n, p in param_dict.items() if _is_embedding(n) or _is_lm_head(n)
            ])
            param_groups = [
                dict(params=hidden_weights, use_muon=True,
                    lr=lr, weight_decay=weight_decay),
                dict(params=hidden_gains_biases+nonhidden_params, use_muon=False,
                    lr=lr_muon, betas=(0.9, 0.95), weight_decay=weight_decay),
            ]
            use_dist = dist.is_available() and dist.is_initialized()
            OptimCls = MuonWithAuxAdam if use_dist else SingleDeviceMuonWithAuxAdam
