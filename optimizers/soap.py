...
class SOAP(optim.Optimizer):
    def __init__(
        ...
        defaults = {
            ...
            "use_znorm": use_znorm,     # 🔹 여기도 추가
        }
        super().__init__(params, defaults)
        self._data_format = data_format

    def _apply_row_znorm(self, grad: torch.Tensor, eps: float = 1e-8):
        """
        Row-wise Z-Norm (LLM 친화 버전):
        - dim >= 2 인 weight tensor에만 적용
        - 각 row(첫 번째 차원)를 뉴런/채널/헤드로 보고,
          row 내부에서 mean/std로 Z-score 정규화 (에너지 보존 없이 순수 ZNorm)
        """
        if grad.dim() < 2:
            # bias, LayerNorm gamma/beta 등 1D 텐서는 그대로 둔다
            return grad

        orig_shape = grad.shape
        # (row, -1) 형태로 펴서 각 row 단위로 처리
        g_flat = grad.view(orig_shape[0], -1)

        row_mean = g_flat.mean(dim=1, keepdim=True)                       # (row, 1)
        row_std  = g_flat.std(dim=1, keepdim=True, unbiased=False)        # (row, 1)
        row_std_safe = torch.clamp(row_std, min=eps)

        g_normed = (g_flat - row_mean) / row_std_safe

        return g_normed.view(orig_shape)
    ...
    def step(self, closure = None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                # 🔹 ZNorm 적용 (옵션)
                if group.get("use_znorm", False):
                    with torch.no_grad():
                        grad = self._apply_row_znorm(grad)
                        p.grad = grad  # 이후 모든 로직에서 ZNorm된 grad 사용
