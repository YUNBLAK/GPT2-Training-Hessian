    ...
    def train(
        ...
        for step in range(max_steps):
            ...
            if self.optimizer_name == "muon":
                lr_muon = self.estimate_lr(step, warmup_steps, max_steps, max_lr_muon, min_lr_muon)
                self.optimizer.param_groups[0]['lr'] = lr        
                self.optimizer.param_groups[1]['lr'] = lr_muon
            else:
    ...
    def evaluate_validation(self, step):
        ...
        if self.master_process:
            ...
            if step > 0 and (step % 10000 == 0 or self.is_last_step):
                ...
                ckpt_path = os.path.join(logdir, f'model_{self.optimizer_name}_{step:05d}.pt')
                checkpoint = {
...
def get_args():
    ...
    parser.add_argument("--mini_batch_size", type=int, default=16, help="setting of mini_batch_size is just a performance optimization. bigger gpu, bigger mini_batch_size")
    ...
    parser.add_argument("--max_lr_muon", type=float, default=3e-3)
    parser.add_argument("--min_lr_muon", type=float, default=3e-3 * 0.1)
    parser.add_argument("--warmup_steps", type=int, default=715)
    parser.add_argument("--weight_decay", type=float, default=0.01)
    parser.add_argument("--optimizer", type=str, default="AdamW")
    parser.add_argument("--num_epochs", type=int, default=1)
    parser.add_argument("--steps_per_epoch", type=int, default=19073)    # 10^10 / 2^19 ~ 19073 for 1 epoch on FineWebEdu-sample10BT default=19073
...
def plot_hessian(model, device, train_loader, args):
    from contextlib import nullcontext
    if hasattr(train_loader, 'tokens'):
        ...
        data_dir = getattr(train_loader, 'data_dir', '/path/to/data')
        train_bin = os.path.join(data_dir, 'train.bin')
        train_data = np.memmap(train_bin, dtype=np.uint16, mode='r').astype(np.int64, copy=False)
    ...
    with sdp_kernel(enable_flash=False, enable_mem_efficient=False, enable_math=True):
        hessian = utils.hessian_spectrum.Hessian(
            model=model,
            ckpt_iteration=0,
...
def main():
    ...
    logpath = os.path.join(args.logdir, f'model_{args.optimizer}_log.txt')
    ...
    optimizer = raw_model.configure_optimizers(weight_decay=args.weight_decay, optm = args.optimizer, lr=args.max_lr, lr_muon=args.max_lr_muon, device_type=device_type, master_process=master_process)
    ...
    trainer = Trainer(model, optimizer, train_loader, val_loader, token_encoder, args.eval_freq, grad_accum_steps, 
                      ddp, ddp_rank, ddp_world_size, device, logpath, optimizer_name=args.optimizer)

    max_steps = args.steps_per_epoch * args.num_epochs
    trainer.train(max_steps, args.warmup_steps, args.max_lr, args.min_lr, args.max_lr_muon, args.min_lr_muon)

    dt = (time.time() - start_time) / (60*60)
    print(f"Total training time: {dt:.4f}hr")

    plot_hessian(model, device, train_loader, args)
